<?php

namespace App\Modules\Example\Repositories {

    use App\Modules\Category\Entity\Category;
    use Core\AbstractResource;
    use Ozone\Upload;

    class CategoryRepository extends AbstractResource
    {

        protected static $path = 'uploads/category/';

        public function findAll($limit, $page, $filter = [])
        {
            $limit = empty($limit) ? getenv('DEF_LIMIT', 12) : $limit;

            $pages = ($page == '') ? 1 : $page;
            $offset = ($pages - 1) * $limit;

            $qb = $this->em->createQueryBuilder();


            $qb->select('e')
                ->from(Category::class, 'e');

            if (array_key_exists('title', $filter) and $filter['title'] != '') {
                $qb->where('e.title LIKE :title')
                    ->setParameter('title', '%' . $filter['title'] . '%');
            }

            // Filter by Status
            $status = empty($filter['status']) ? [0, 1] : $filter['status'];
            $qb->andWhere('e.status IN (:status)')
                ->setParameter('status', $status);

            // Filter by Date
            if (array_key_exists('from', $filter) and $filter['to'] != '') {

                $qb->andWhere('e.created_at BETWEEN :from AND :to')
                    ->setParameter('from', $filter['from'])
                    ->setParameter('to', $filter['to']);
            }

            // Sort By Order
            $sort = empty($filter['sort']) ? 'id' : $filter['sort'];
            $order = empty($filter['order']) ? 'DESC' : $filter['order'];
            $qb->orderBy('e.' . $sort, $order);

            $qb->setMaxResults($limit);
            $qb->setFirstResult($offset);

            return $qb->getQuery()->getResult();
        }

        public function save(Category $category)
        {
            try {

                $this->em->persist($category);
                $this->em->flush();
                return $category->getId();

            } catch (\Throwable $t) {
                return false;
            }

        }

        public function update($data)
        {
            try {

                $this->em->merge($data);
                $this->em->flush();
                return true;

            } catch (\Throwable $t) {
                return false;
            }

        }

        public function delete($data)
        {
            try {

                if (is_array($data)) {
                    foreach ($data as $val) {
                        $this->em->remove($val);
                    }
                } else {
                    $this->em->remove($data);
                }

                $this->em->flush();
                return true;

            } catch (\Throwable $t) {
                return false;
            }
        }

        public function find($id)
        {
            $example = $this->em->getRepository(Category::class);
            return $example->find($id);
        }

        public function upload($file, $rename = true)
        {
            return Upload::move(self::$path, $file, $rename);
        }

    }
}
